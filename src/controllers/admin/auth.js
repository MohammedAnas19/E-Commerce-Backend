...
exports.signup=(req, res) => {
    ...
    .exec((error,user)=>{
      ...
      _user.save((error,data)=>{
        ...
        if(data){return res.status(201).json({message:"Admin Created SUCCESSFULLY"});
        }
      });
    });  
}
    ...
    .exec((error,user)=>{
        ...
        if(user) {
            if(user.authenticate(req.body.password) && user.role === 'admin'){
                ...
                return res.status(400).json({message:user.role !== 'admin'?"You are not Authorized ":"INCORRECT Username/Password"});
            }
        }
        else{
